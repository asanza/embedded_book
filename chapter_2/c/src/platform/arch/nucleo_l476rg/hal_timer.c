...
static void tim_config_period_us(int timer_instance, uint32_t period_us) {
    ...
    if (timer_instance == 0) {
        CR1 = &TIM2_CR1; DIER = &TIM2_DIER; SR = &TIM2_SR; PSC = &TIM2_PSC; ARR = &TIM2_ARR; CNT = &TIM2_CNT; EGR = &TIM2_EGR;
    } else if (timer_instance == 1) {
        CR1 = &TIM3_CR1; DIER = &TIM3_DIER; SR = &TIM3_SR; PSC = &TIM3_PSC; ARR = &TIM3_ARR; CNT = &TIM3_CNT; EGR = &TIM3_EGR;
    } else {
        CR1 = &TIM4_CR1; DIER = &TIM4_DIER; SR = &TIM4_SR; PSC = &TIM4_PSC; ARR = &TIM4_ARR; CNT = &TIM4_CNT; EGR = &TIM4_EGR;
    }
...
void hal_timer_init(int timer_instance, uint32_t period_us, bool one_shoot) {
    ...
    tim_config_period_us(timer_instance, (s_period_us_arr[timer_instance] == 0) ? 1U : s_period_us_arr[timer_instance]);

    /* Enable timer IRQ in NVIC for the corresponding timer. */
    if (timer_instance == 0) NVIC_ISER0 |= TIM2_IRQ_BIT;
    else if (timer_instance == 1) NVIC_ISER0 |= TIM3_IRQ_BIT;
    else NVIC_ISER0 |= TIM4_IRQ_BIT;
}
